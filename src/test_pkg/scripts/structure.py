...
## Structure 3 #####################################################
class Config3:                  # four magnets configuration
    ...
    def __init__(
        ...
        # # case 1
        # if self.number_magnets >= 4:                        
        #     self.rL.update({
        #         '1': np.array([[0.0], [+d], [0.0]]),
        #         '2': np.array([[-d*np.cos(np.pi/6)], [-d*np.sin(np.pi/6)], [0.0]]),
        #         '3': np.array([[+d*np.cos(np.pi/6)], [-d*np.sin(np.pi/6)], [0.0]]),
        #         })
        #     self.mL.update({
        #         '1': np.array([[-m], [0.0], [0.0]]),
        #         '2': np.array([[+m*np.cos(np.pi/3)], [-m*np.sin(np.pi/3)], [0.0]]),
        #         '3': np.array([[+m*np.cos(np.pi/3)], [+m*np.sin(np.pi/3)], [0.0]]),
        #         })    
        ...
        #     self.mL.update({
        #         '1': np.array([[0.0], [+m], [0.0]]),
        #         '2': np.array([[-((m/2)/np.cos(np.pi/3))*np.cos(np.pi/6)], [-((m/2)/np.cos(np.pi/3))*np.sin(np.pi/6)], [0.0]]),
        #         '3': np.array([[+((m/2)/np.cos(np.pi/3))*np.cos(np.pi/6)], [-((m/2)/np.cos(np.pi/3))*np.sin(np.pi/6)], [0.0]]),
        #         })    
